...
def _get_matching_file(
    ...
    min_digits: int=0,
    ignore_ext: bool=False) -> Union[str, Dict[str,str]]:
    ...
    if len(matching_files):
        ...
        if target == 'next':
            return os.path.join(dir_, base + "_{{:0>{}d}}".format(min_digits).format(maxnum+1) + suffix)
        return {k:os.path.join(dir_,v) for k,v in matching_files.items()}
    elif target == 'last':
        raise FileNotFoundError('No file of format {} exists'.format(base + '_xxx' + suffix))
    else:
        if target == 'next':
...
def get_last(name, zero_ext=False, ignore=[],ignore_ext=False):
    ...
    return _get_matching_file(name, 'last', zero_ext=zero_ext, ignore=ignore,ignore_ext=ignore_ext)


def safe_save(name, zero_ext=False, verbose=False, min_digits=0,ignore_ext=False):
    name = _get_matching_file(name, 'next', zero_ext=zero_ext, ignore=[],min_digits=min_digits,ignore_ext=ignore_ext)
    if verbose:
        print('\n\nSaving in:\n\t{}\n\n'.format(name))
    return name
