...
class Experiment:
    ...
    def from_conn(cls, *, conn: Connection, table_name: str, id: str) -> 'Experiment':
        ...
        """
        cursor = conn.execute(f"SELECT id, status, config, metrics, metadata, error FROM {escape_identifier(table_name)} WHERE id = ?", (id,))
        data = cursor.fetchone()
        return cls.deserialize(data)
    
    def upsert(self, *, conn: Union[Connection, None]=None, table_name: Union[str, None]=None) -> None:
        conn.execute(
            upsert_query(table_name),
            self.as_dict()
        )
        conn.commit()

    def update(self, *, 
        conn: Union[Connection, None]=None, table_name: Union[str, None]=None, 
        status: Union[str, None] = None, metrics: Union[Dict[str, Any], None] = None, 
        error: Union[str, None] = None, metadata: Union[Dict[str, Any], None] = None,
    ) -> None:
        ...
        if do_update:
            ...
            conn.execute(f"UPDATE {escape_identifier(table_name)} SET {update_clause} WHERE id = :id", {k: to_sqlite_compatible(v) for k, v in data.items()})
            conn.commit()
    ...
    def update_status(self, *, 
        ...
        conn, table_name = self.get_conn(conn=conn, table_name=table_name)
        self.status = status
        conn.execute(f"UPDATE {escape_identifier(table_name)} SET status = :status WHERE id = :id", {"status": to_sqlite_compatible(status), "id": self.id})
        conn.commit()
    ...
    def update_metrics(self, *, 
        ...
        conn, table_name = self.get_conn(conn=conn, table_name=table_name)
        self.metrics.update(metrics)
        conn.execute(f"UPDATE {escape_identifier(table_name)} SET metrics = :metrics WHERE id = :id", {"metrics": to_sqlite_compatible(metrics), "id": self.id})
        conn.commit()
    ...
    def update_error(self, *, 
        ...
        conn, table_name = self.get_conn(conn=conn, table_name=table_name)
        self.error = error
        conn.execute(f"UPDATE {escape_identifier(table_name)} SET error = :error WHERE id = :id", {"error": to_sqlite_compatible(error), "id": self.id})
        conn.commit()
    ...
    def update_metadata(self, *, 
        ...
        conn, table_name = self.get_conn(conn=conn, table_name=table_name)
        self.metadata.update(metadata)
        conn.execute(f"UPDATE {escape_identifier(table_name)} SET metadata = :metadata WHERE id = :id", {"metadata": to_sqlite_compatible(metadata), "id": self.id})
        conn.commit()
    ...
    def get_conn(self, *, conn: Union[Connection, None]=None, table_name: Union[str, None]=None) -> Tuple[Connection, str]:
        ...
        if experiment_set is None:
            raise ValueError("Experiment is not bound to an experiment set. Cannot get connection.")
...
class ExperimentSet:
    ...
    def ensure_table(conn: Connection, table_name: str) -> None:
        conn.execute(f"CREATE TABLE IF NOT EXISTS {escape_identifier(table_name)} (id TEXT PRIMARY KEY, status TEXT, config JSONB, metrics JSONB, metadata JSONB, error TEXT)")
    
    @classmethod
    def from_conn(cls, *, conn: Connection, table_name: str) -> 'ExperimentSet':
        cls.ensure_table(conn, table_name)
        cursor = conn.execute(f"SELECT id, status, config, metrics, metadata, error FROM {escape_identifier(table_name)}")
    ...
    def get_by_status(self, status: Union[str, ExperimentStatus]) -> List[Experiment]:
        ...
        return [experiment for experiment in self.experiments.values() if experiment.status == status]
    
    def get_all(self) -> List[Experiment]:
        return list(self.experiments.values())
    
    def get_incomplete(self) -> List[Experiment]:
        return [experiment for experiment in self.experiments.values() if experiment.status != ExperimentStatus.COMPLETED]
    ...
    def pull(self) -> None:
        cursor = self.conn.execute(f"SELECT id, status, config, metrics, metadata, error FROM {escape_identifier(self.table_name)}")
        count = 0
        for row in cursor:
