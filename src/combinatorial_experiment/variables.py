...
class Variable(object):
    ...
    def record(self):
        if self._record:
            if isinstance(self._record, Iterable) and not isinstance(self._record, six.string_types):
...
class EKFoldVariable(Variable):
    def __init__(self,fold):
        super().__init__('efold',range(fold),f'ext{fold}fold',record=True,
                         dict={'ekfold':fold})
...
class IKFoldVariable(Variable):
    def __init__(self,fold):
        super().__init__('ifold',range(fold),f'int{fold}fold',record=True,
                         dict={'ikfold':fold})
...
class ProductVariable(VariableCollection):
    ...
    def __iter__(self):
        if self._experiments is not None:
            ...
            if self._experiments < len(it):
                self._iterator = iter([it[el] for el in np.random.choice(len(it),
                                                       size=self._experiments,
                                                       replace=False)])
            else:
    ...
    def __len__(self):
        ...
        return np.product(lengths) if not self._experiments else min(np.product(lengths),self._experiments)

    def serialize(self):
        ser = super().serialize()
        ser[type(self).__name__].update({'experiments':self._experiments,
...
class RepeatVariable(VariableCollection):
    ...
    def __init__(self, variables,times=None):
        ...
        super(RepeatVariable, self).__init__(
            lambda v: itertools.chain.from_iterable(itertools.repeat(x, self.times) for x in v), variables)
...
deserializable_vars = dict(
    ...
    **dict.fromkeys(['efold','extfold','ExtFold','ExtFoldVariable',
                     'ekfold','extkfold','ExtKFold','ExtKFoldVariables',
                     'fold','Fold','FoldVariable','KFold','KFoldVariable'],EKFoldVariable),
    **dict.fromkeys(['ifold','intfold','IntFold','IntFoldVariable',
                     'ikfold','intkfold','IntKFold','IntKFoldVariable'],IKFoldVariable),
    **dict.fromkeys(['zip','Zip','zipped','Zipped','ZippedVariable'],ZippedVariable),
    **dict.fromkeys(['product','Product','ProductVariable'],ProductVariable),
    **dict.fromkeys(['chain','Chain','ChainVariable'],ChainVariable))
            
            
reserved_kwargs = ['experiments',
                   'experiment_seed',
                   'times']
